...
class PDF_MySQL_Table extends \Fpdf\Fpdf
{
    ...
    {
        // Execute query
        $res = DB::queryRaw($query) or die("<h1>C'è stato un errore durante la generazione del PDF. Contattare un amministratore</h1>");
        ...
        foreach ($this->aCols as $i => $col) {
            if ($col['c'] == '') {
                if (is_string($col['f']))
                    ...
                    // Get column name
                    $this->aCols[$i]['c'] = ucfirst(mysqli_fetch_field_direct($res, $i)->name);
            }
        }
        ...
        if(mysqli_num_rows($res) > 0){
            ...
            while ($row = mysqli_fetch_row($res)){
                // Get row saved year
                $year = DateTime::createFromFormat("d/m/Y", $row[1])->format("Y");

                if($year != $pre_year && !is_bool($pre_year)){
                    //echo "Color";
                    // Add separator
                    $this->ColoredRow(array("r"=>255,"g"=>0,"b"=>0),$prop["width"]);
    ...
    function ColoredRow($arr,$w,$h=5){
        $this->SetX($this->TableX);

        // Set color
        $this->SetFillColor($arr['r'],$arr['g'],$arr['b']);

        $this->Cell($w, $h, "", 1, 0, 'C', 1);
        $this->Ln();
    }
}
